...
在移动端，前缀会优先匹配你输入的字母，一旦命中你预先设置的前缀，则这个前缀的内容会上屏（或者触发对应的宏）。如果没有前缀命中，键盘则会尝试获取输入框中光标左侧的最多10个字从前往后按长度优先匹配你的前缀，如果还是没有命中，则会使用默认的普通点划内容。

> 前缀内容也支持宏。前缀优先级总结：1. 字母前缀完全匹配 2. 上文前缀最长匹配 3. 普通点划
