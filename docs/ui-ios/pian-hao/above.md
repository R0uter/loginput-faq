...
九宫格键盘左侧有一列快捷符号，点一下就能输入。默认是 `，`、`。`、`？`、`！`、`……`、`@`、`%` 这七个，你可以按自己的习惯改掉。

进入设置后，点击任意条目可以编辑内容；点"新增符号"可以添加新的，最多七项；左划条目可以删除，但至少要留一项；编辑模式下拖动右侧手柄可以调整顺序。如果改乱了，点"恢复默认"就能恢复原来的内容。
...
你可以通过滑块微调键盘按键上文字的大小，正数增大负数减小，默认为 0 即保持原始大小不变。
