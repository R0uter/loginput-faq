...
将当前双拼输入编码转换为全拼文本并直接上屏（清空编码区，不进入候选）。

- 如果当前有候选词，取第一个候选词的拼音作为全拼文本
- 如果没有候选词，将双拼编码通过方案映射转为全拼
- 转换结果作为普通文本插入光标处

适用场景：双拼输入过程中想直接把全拼拼写打出来。
...
> 注意：受到 iOS 系统限制，该宏仅能打开网页链接，以及系统自带的 URL Scheme，无法打开第三方 APP 的 URL Scheme。
...
切换落格输入法“在输入框显示输入字母”的兼容模式，或许可以使其在闲鱼等 APP 中正常输入而不需要在候选栏显示输入字母。
