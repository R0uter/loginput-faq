...
落格输入法默认提供“落格解释库”，它由 空山新雨 进行维护和更新，通过以下几项优化使其比经典解释库更高效和准确：
（1）对于一个字有多个解释的，只保留一个最常见的解释；
（2）对于解释词含混晦涩、指向不明的，主要采用在原词加上一两个字的方式来确定它的唯一性；
（3）对于繁体字的解释与简体不同的，全部改为简体字的解释）；
（4）对于繁体和异体字，将其“繁体”和“异体”的说明移到解释词之后；
（5）对于一些字词“婷婷”发音错误的，用同音字纠正其读音；
（6）对于一些专业用字的解释（比如偏旁部首和化学元素等）也进行了规范和统一。
如果你想要和其他输入法一样的解释，请选择经典解释库。
